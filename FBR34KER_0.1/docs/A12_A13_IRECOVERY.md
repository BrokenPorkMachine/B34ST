...
FBR34KER 0.2.3 adds an optional host workflow for sending a validated recovery boot image through the `irecovery` utility from libirecovery. The workflow targets devices or development boards that the operator owns or is authorized to test.
...
| A12X/A12Z | T8027 | `0x8027` | `profiles/apple-a12x-recovery.json` |
| A13 | T8030 | `0x8030` | `profiles/apple-a13-recovery.json` |

The A12X/A12Z entry is marked experimental-profile-only.
