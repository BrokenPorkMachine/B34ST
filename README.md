...
make sdk-release      # produce dist/B34ST_0.4.5b_Beta_operational.zip
...
Produces \`dist/B34ST_0.4.5b_Beta_operational.zip\` containing:
...
Version 0.4.5b includes bounded in-memory models for patch, boot-policy, and persistence concepts. They exist to validate interface shape, status output, policy gates, event wiring, and failure handling. They do not modify target memory, Apple trust policy, filesystems, or reboot state.
