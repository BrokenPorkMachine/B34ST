...
```
FBR34KER 0.3.0-beta — environment diagnostics
  [OK]   clang — found
  [OK]   ld.lld — found
...
> This section describes the full exploit chain for physical devices. The public operational release does not include the `kernel/`, `arch/`, `platform/`, or `host/` source code — the exploit and jailbreak internals are private. Pre-built firmware images in `build-*` contain the compiled exploit chain.
...
| Archive | Contents |
|---|---|
| `FBR34KER_0.3.0_Beta_operational_<date>.zip` | Public release — no private code |
...
| Archive | Contents |
|---|---|
| `FBR34KER_0.3.0_Beta_source_<date>.zip` | All source code, docs, scripts (excludes build artifacts) |
| `FBR34KER_0.3.0_Beta_complete_<date>.zip` | Source + all build artifacts, SDK, boot images, simulations |
...
| `docs/B34ST_TASKS.md` | Historical B34ST task list (all complete as of 0.3.0) |
