...
def mk(cve_id, comps, desc, versions, goals, etype, severity, pv="", refs=None, creds=None):
    return {
        ...
        "mitigations": [], "patch_version": pv, "published": "",
        "references": refs or [], "credits": creds or [], "chainable_with": [],
    }
...
goals = Counter()
for c in db["cves"]:
    for co in c["affected_components"]: comps[co] += 1
    for g in c["goals"]: goals[g] += 1
print(f"\nComponents ({len(comps)}):")
for k,v in comps.most_common(30): print(f"  {k:30s}: {v:3d}")
print(f"\nGoals ({len(goals)}):")
for k,v in goals.most_common(30): print(f"  {k:30s}: {v:3d}")
