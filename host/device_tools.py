...
from irecovery_boot import (DeviceInfo, RecoveryError, load_device_info,
                            query_device, validate_target)


def tool_version(path: str) -> str:
    try:
        result = subprocess.run([path, "--version"], text=True, stdout=subprocess.PIPE,
...
def executable_path(value: str) -> str | None:
    return shutil.which(value) if pathlib.Path(value).name == value else (value if pathlib.Path(value).is_file() else None)


def discover_profiles(directory: pathlib.Path, device: DeviceInfo) -> list[dict[str, object]]:
    ...
    for path in sorted(directory.glob("apple-*-recovery.json")):
        try:
            ...
            continue
        matches.append({"profile_id": profile["profile_id"], "path": str(path),
                        "exact": bool(profile.get("requires_exact_product", False)),
                        "maturity": profile.get("maturity", "unverified")})
    return sorted(matches, key=lambda item: (not bool(item["exact"]), str(item["profile_id"])))
...
def list_devices(args: argparse.Namespace) -> int:
    ...
    exact = sum(1 for value in values for profile in value["profile_matches"] if profile["exact"])
    output = {"schema_version": 1, "count": len(values), "exact_profile_matches": exact,
              "devices": values}
...
def watch_devices(args: argparse.Namespace) -> int:
    ...
    for iteration in range(args.iterations):
        try:
            ...
            current = None
        if current != previous:
            event = "connected" if previous is None and current else "disconnected" if current is None else "changed"
            print(json.dumps({"schema_version": 1, "iteration": iteration + 1,
                              "event": event, "device": current}, sort_keys=True), flush=True)
        previous = current
...
def doctor_device(args: argparse.Namespace) -> int:
    executable = executable_path(args.irecovery)
    checks = [{"name": "host-macos", "ok": platform.system() == "Darwin", "required": False,
               "detail": platform.platform()},
              {"name": "irecovery", "ok": bool(executable), "required": not args.device_info,
               "detail": tool_version(executable) if executable else "not installed"}]
    device = load_device_info(args.device_info[0]) if args.device_info else (query_device(executable, args.ecid, args.timeout) if executable else None)
    profile = load_profile(args.profile) if args.profile else None
    if profile and device:
        validate_target(profile, device)
    matches = discover_profiles(args.profiles_dir, device) if device else []
    checks.append({"name": "device-query", "ok": device is not None, "required": True,
                   "detail": device.public_dict() if device else "no device evidence"})
    checks.append({"name": "profile-match", "ok": profile is not None and device is not None,
                   "required": bool(args.profile), "detail": profile.get("profile_id") if profile else "profile not requested"})
    checks.append({"name": "exact-profile-available", "ok": any(item["exact"] for item in matches),
                   "required": False, "detail": matches})
    passed = all(item["ok"] for item in checks if item["required"])
    print(json.dumps({"schema_version": 1, "passed": passed, "checks": checks,
                      "device": device.public_dict() if device else None,
                      "profile": profile.get("profile_id") if profile else None,
                      "profile_matches": matches}, indent=2, sort_keys=True))
...
def parser() -> argparse.ArgumentParser:
    ...
    doctor = sub.add_parser("doctor"); add_common(doctor)
    doctor.add_argument("--profile", type=pathlib.Path)
    doctor.add_argument("--profiles-dir", type=pathlib.Path, default=pathlib.Path("profiles"))
    listing = sub.add_parser("list"); add_common(listing)
    listing.add_argument("--profiles-dir", type=pathlib.Path, default=pathlib.Path("profiles"))
    listing.add_argument("--allow-empty", action="store_true")
    watch = sub.add_parser("watch"); add_common(watch)
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if args.command == "doctor": return doctor_device(args)
        if args.command == "list": return list_devices(args)
        if args.iterations < 1 or args.iterations > 10000 or args.interval < 0 or args.interval > 300:
