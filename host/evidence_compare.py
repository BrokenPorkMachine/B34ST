...
def main(argv=None):
    ...
    try:
        ...
        fields = {
            ...
            "device_cpid": ("device", "cpid"),
            "device_ecid": ("device", "ecid"),
        ...
        for name, path in fields.items():
            ...
            if x != y:
                changes.append({"field": name, "first": x, "second": y})
        ls = {
            item.get("stage"): item.get("status")
            for item in left.get("stages", [])
            if isinstance(item, dict)
        }
        rs = {
            item.get("stage"): item.get("status")
            for item in right.get("stages", [])
            if isinstance(item, dict)
        }
        for stage in sorted(set(ls) | set(rs)):
            if ls.get(stage) != rs.get(stage):
                changes.append(
                    {
                        "field": f"stage:{stage}",
                        "first": ls.get(stage),
        ...
        print(
            json.dumps(
                {"schema_version": 1, "equal": not changes, "changes": changes},
                indent=2,
                sort_keys=True,
            )
        )
        ...
        print(f"evidence compare error: {exc}", file=sys.stderr)
        return 2
