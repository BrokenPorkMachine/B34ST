...
from deployment_transport import (
    FaultPlan,
...
def make_client(
    args: argparse.Namespace, profile_path: pathlib.Path
) -> tuple[DeploymentClient, object]:
    profile = load_profile(profile_path)
    if args.simulator_state is not None or not any((args.tcp, args.unix, args.serial)):
        state = args.simulator_state or pathlib.Path(
            "runtime-artifacts/deployment-target"
        )
        ...
        if args.fault_plan is not None:
            raise TransportError(
                "--fault-plan is available only with --simulator-state"
            )
        if args.tcp:
...
def local_artifacts(
    values: Sequence[tuple[str, str, pathlib.Path, int]],
) -> list[LocalArtifact]:
    return [
        LocalArtifact.from_path(name, kind, path, address)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    plan = subparsers.add_parser(
        "plan", help="validate a deployment without contacting a target"
    )
    ...
    deploy = subparsers.add_parser(
        "deploy", help="stage, verify, start, and collect evidence"
    )
    deploy.add_argument("--profile", type=pathlib.Path, required=True)
    deploy.add_argument(
        "--artifact", action="append", type=artifact_value, required=True
    )
    deploy.add_argument(
        "--authorize",
        action="store_true",
    ...
    resume_group.add_argument(
        "--no-resume", action="store_true", help="always begin a new deployment session"
    )
    ...
    inspect = subparsers.add_parser(
        "inspect", help="read target identity and capabilities"
    )
    ...
    reset = subparsers.add_parser(
        "reset", help="reset a deployment session, preserving evidence"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
                ...
                json.dumps(
                    {
                        ...
                        "artifacts": [
                            artifact.manifest_record() for artifact in artifacts
                        ],
                    },
                    indent=2,
                    sort_keys=True,
        ...
        if args.command == "inspect":
            print(
                json.dumps(
                    {"hello": client.hello(), "capabilities": client.capabilities()},
                    indent=2,
                    sort_keys=True,
                )
            )
        ...
        if args.command == "deploy":
            ...
            result = client.deploy(
                ...
                resume=args.recover or not args.no_resume,
                start=not args.stage_only,
            )
            evidence = client.evidence()
            archive = write_evidence_bundle(
                args.evidence_dir,
                profile=profile,
                artifacts=artifacts,
                evidence=evidence,
