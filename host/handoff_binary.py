...
class HandoffBinaryError(ValueError):
    pass
...
def _pack_handoff(doc: dict[str, Any], region_offset: int) -> bytes:
    monitor = doc["monitor"]
    fb = doc.get("framebuffer") or {}
    policy = doc.get("module_policy") or {}
    flags = 0
    if fb:
        flags |= 1 << 2
    if policy:
    ...
    if capmask is None:
        capmask = sum(MODULE_CAPS[x] for x in policy.get("allowed_capabilities", []))
    out = bytearray()
    out += struct.pack(
        "<QIIQQQIIQQQQ",
        HANDOFF_MAGIC,
    ...
    out += struct.pack(
        ...
        fb.get("width", 0),
        fb.get("height", 0),
        fb.get("pixels_per_row", 0),
        fb.get(
            "pixel_format_id", PIXEL_FORMATS.get(fb.get("pixel_format", "xrgb8888"), 0)
        ),
        0,
    )
    ...
    out += struct.pack(
        ...
        0,
        0,
        doc["entry_exception_level"],
        doc["page_granule"],
        doc["boot_cpu_id"],
        _n(fb.get("size", 0)),
        fb.get("bytes_per_pixel", 0),
        fb.get("rotation", 0),
        capmask,
        policy.get("dynamic_slots", 0),
        0,
        policy.get("instruction_budget", 0),
        _n(doc["firmware_revision"]),
        0,
        0,
    )
    if len(out) != HANDOFF_SIZE:
...
def build_blob(path: pathlib.Path) -> bytes:
    ...
    regions = b"".join(
        REGION.pack(
            ...
            _n(r["size"]),
            REGION_TYPES[r["type"]],
            sum(ATTR_BITS[x] for x in r["attributes"]),
        )
    ...
    payload[
        handoff_offset - HEADER.size : handoff_offset - HEADER.size + len(handoff)
    ] = handoff
    payload[
        regions_offset - HEADER.size : regions_offset - HEADER.size + len(regions)
    ] = regions
...
def inspect_blob_bytes(data: bytes, source: str = "<memory>") -> dict[str, Any]:
    if len(data) < HEADER.size:
        raise HandoffBinaryError("handoff blob is shorter than its header")
    values = HEADER.unpack_from(data)
    magic, fmt, abi = values[:3]
    if magic != MAGIC or fmt != FORMAT_VERSION or abi != ABI_VERSION:
        raise HandoffBinaryError("unsupported handoff blob header")
    (
        header_size,
        total,
        json_offset,
    ...
    if header_size != HEADER.size or total != len(data) or handoff_size != HANDOFF_SIZE:
        raise HandoffBinaryError("handoff blob sizes are inconsistent")
    if hashlib.sha256(data[HEADER.size :]).digest() != digest:
        raise HandoffBinaryError("handoff blob SHA-256 mismatch")

    def covered(offset: int, size: int) -> bool:
        return HEADER.size <= offset <= len(data) and 0 <= size <= len(data) - offset

    if (
        not covered(json_offset, json_size)
        or not covered(handoff_offset, handoff_size)
        or not covered(regions_offset, region_count * REGION.size)
    ):
        raise HandoffBinaryError("handoff blob section is outside the file")
    try:
        doc = json.loads(data[json_offset : json_offset + json_size].decode())
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise HandoffBinaryError(f"embedded JSON is invalid: {exc}") from exc
    ...
    region_ptr = struct.unpack_from("<Q", raw, 32)[0]
    count = struct.unpack_from("<I", raw, 40)[0]
    if (
        magic_value != HANDOFF_MAGIC
        or version != 4
        or structure_size != HANDOFF_SIZE
        or region_ptr != regions_offset
        or count != region_count
    ):
        raise HandoffBinaryError("embedded handoff header or offset pointer is invalid")
    regions = []
    for i in range(region_count):
        base, size, typ, attrs = REGION.unpack_from(
            data, regions_offset + i * REGION.size
        )
        regions.append(
            {
                "base": f"0x{base:x}",
    ...
    return {
        ...
        "service_declaration_mask": service_mask,
        "runtime_declaration_mask": runtime_mask,
        "sections": {
            "json": {"offset": json_offset, "size": json_size},
            "handoff": {"offset": handoff_offset, "size": handoff_size},
            "regions": {
                "offset": regions_offset,
                "count": region_count,
                "entry_size": REGION.size,
            },
        },
        "runtime_ready": service_mask == 0 and runtime_mask == 0,
        "normalized": normalized,
        "regions_binary": regions,
    }
...
def load_and_validate_document(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        p = pathlib.Path(td) / "h.json"
        p.write_text(json.dumps(doc), encoding="utf-8")
        return load_and_validate(p)


def inspect_blob(path: pathlib.Path) -> dict[str, Any]:
    return inspect_blob_bytes(path.read_bytes(), source=str(path))


def roundtrip_blob(path: pathlib.Path) -> dict[str, Any]:
    first = inspect_blob(path)
    original = path.read_bytes()
    import tempfile

    with tempfile.TemporaryDirectory() as td:
        j = pathlib.Path(td) / "handoff.json"
        j.write_text(json.dumps(first["normalized"], indent=2), encoding="utf-8")
        rebuilt = build_blob(j)
    return {
        "roundtrip_equal": rebuilt == original,
