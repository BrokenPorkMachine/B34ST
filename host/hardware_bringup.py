...
from first_stage_adapter import (AdapterError, BridgeFirstStageAdapter,
...
def _adapter(args: argparse.Namespace, device: DeviceInfo):
    if getattr(args, "bridge_command", None):
        return BridgeFirstStageAdapter(shlex.split(args.bridge_command), timeout=args.timeout)
    if args.adapter_command:
        return CommandFirstStageAdapter(shlex.split(args.adapter_command), timeout=args.timeout)
    regions = None
    if args.memory_map:
    ...
    return SimulatorFirstStageAdapter(args.state_dir, device.public_dict(), regions=regions,
...
def _transcript(collected: dict[str, object]) -> list[dict[str, object]]:
    ...
    values = state.get("transcript", []) if isinstance(state, dict) else collected.get("transcript", [])
...
def _bundle_values(summary: dict[str, object], profile: dict[str, object],
                   image: dict[str, object], collected: dict[str, object],
                   console_lines: list[str]) -> dict[str, Any]:
    transcript = _transcript(collected)
    transfer_log = "".join(json.dumps(item, sort_keys=True) + "\n" for item in transcript)
    boot = next((item.get("details", {}) for item in summary.get("stages", [])
                 if isinstance(item, dict) and item.get("stage") == "boot-evidence"), {})
    crash = ({"schema_version": 1, "failure": summary.get("failure"),
              "profile_id": summary.get("profile_id"), "device": summary.get("device")}
...
def run_session(args: argparse.Namespace) -> int:
    ...
    if _external(args) and not profile.get("requires_exact_product", False) and not args.allow_family_profile:
        raise BringupError("external hardware sessions require an exact product profile; use --allow-family-profile only for controlled development")
    ...
    if not args.acknowledge_unsigned_code:
        raise BringupError("starting an external monitor requires --acknowledge-unsigned-code")
    ...
    try:
        ...
        if profile.get("requires_exact_product") and adapter_device.get("product") != device.product:
        ...
        for stage in profile.get("required_bringup_stages", DEFAULT_STAGES):
            ...
            try:
                ...
                failed = str(exc)
                stages.append({"stage": failed_stage, "status": "failed", "detail": failed})
                break
        ...
        session_material = "|".join([
            str(profile["profile_id"]), str(device.ecid or "unknown"),
            str(image_public["sha256"]), str(identity.session_generation),
        ])
        summary = {
            "schema_version": 1,
            "project": "FBR34KER",
        ...
        if args.evidence:
            write_session_bundle(args.evidence, _bundle_values(summary, profile, image_public,
                                                                collected, console_lines))
...
def collect_session(args: argparse.Namespace) -> int:
    ...
    try:
        collected = adapter.collect()
        console_lines = _console(adapter, collected)
        summary = {"schema_version": 1, "project": "FBR34KER",
                   "release_version": RELEASE_VERSION, "operation": "bringup-collect",
                   "session_id": hashlib.sha256(json.dumps(collected, sort_keys=True).encode()).hexdigest()[:24],
                   "device": device.public_dict(), "profile_id": None, "passed": True,
                   "stages": [], "collected": collected}
        write_session_bundle(args.output, _bundle_values(summary, {}, {}, collected, console_lines))
        print(args.output)
        return 0
    finally:
        adapter.close()
...
def parser() -> argparse.ArgumentParser:
    ...
    adapters.add_argument("--adapter-command", help="one-shot authorized adapter executable; JSON request on stdin")
    adapters.add_argument("--bridge-command", help="persistent authorized bridge command using JSON-lines protocol v1")
    common.add_argument("--memory-map", type=pathlib.Path,
    ...
    run.add_argument("--inject-failure", choices=["verify-image", "upload", "start", *DEFAULT_STAGES])
    run.add_argument("--recover-on-failure", action=argparse.BooleanOptionalAction, default=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "run": return run_session(args)
        if args.command == "collect": return collect_session(args)
        return recover_session(args)
    except (BringupError, AdapterError, RecoveryError, BootImageError, OSError,
            ValueError, json.JSONDecodeError, SessionBundleError) as exc:
