...
ALLOWED_CONSOLES = {
    "any", "callback", "pl011", "memory-ring", "framebuffer", "semihosting", "none"
}
ALLOWED_INTERRUPTS = {"any", "gicv2", "gicv3", "callback", "polling", "none"}
ALLOWED_TIMERS = {"arm-generic", "callback", "any", "none"}
ALLOWED_SERVICES = {
    "console_write", "console_read", "console_flush", "monotonic_time",
    "power", "framebuffer", "framebuffer_flush", "interrupt_controller",
    "watchdog_configure", "watchdog_kick", "device_tree", "memory_map",
}
...
def _require_string(document: dict[str, Any], key: str, *, max_length: int = 96) -> str:
    ...
    if not isinstance(value, str) or not value.strip() or len(value) > max_length:
        raise HardwareProfileError(f"{key} must be a non-empty string of at most {max_length} characters")
    return value.strip()
...
def validate_profile(document: dict[str, Any]) -> dict[str, Any]:
    ...
    unknown = set(document) - {
        "schema_version", "name", "architecture", "expected_el", "console",
        "interrupt_controller", "timer", "framebuffer", "required_services", "notes", "review",
    }
    if unknown:
        raise HardwareProfileError(f"unknown profile field(s): {', '.join(sorted(unknown))}")
    ...
    if not isinstance(expected_el, int) or isinstance(expected_el, bool) or expected_el not in range(4):
    ...
    interrupts = _require_string(document, "interrupt_controller", max_length=32).lower()
    if interrupts not in ALLOWED_INTERRUPTS:
        raise HardwareProfileError(f"unsupported interrupt_controller value {interrupts!r}")
    ...
    if framebuffer is not None:
        ...
        unknown_fb = set(framebuffer) - {"required", "pixel_formats", "minimum_width", "minimum_height"}
        if unknown_fb:
            raise HardwareProfileError(f"unknown framebuffer field(s): {', '.join(sorted(unknown_fb))}")
        ...
        if not isinstance(pixel_formats, list) or not all(isinstance(item, str) for item in pixel_formats):
            raise HardwareProfileError("framebuffer.pixel_formats must be a string list")
        minimum_width = framebuffer.get("minimum_width", 0)
        minimum_height = framebuffer.get("minimum_height", 0)
        if not isinstance(minimum_width, int) or minimum_width < 0 or minimum_width > 65535:
            raise HardwareProfileError("framebuffer.minimum_width must be 0..65535")
        if not isinstance(minimum_height, int) or minimum_height < 0 or minimum_height > 65535:
    ...
    if unknown_services:
        raise HardwareProfileError(f"unknown required service(s): {', '.join(sorted(unknown_services))}")
    ...
    if review is not None:
        ...
        allowed_review = {"source", "compatibility_result", "observed", "supported_services", "untested_services"}
        unknown_review = set(review) - allowed_review
        if unknown_review:
            raise HardwareProfileError(f"unknown review field(s): {', '.join(sorted(unknown_review))}")
        observed = review.get("observed", {})
        if not isinstance(observed, dict) or not all(isinstance(k, str) and isinstance(v, str) and v in VALID_STATES for k, v in observed.items()):
            raise HardwareProfileError("review.observed must map feature names to valid states")
        ...
        review = {
            ...
            "compatibility_result": str(review.get("compatibility_result", "partial")).lower(),
...
def write_profile_template(path: pathlib.Path) -> None:
    ...
    path.write_text(json.dumps(template_profile(), indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def _detect_console(text: str) -> str:
    lower = text.lower()
    if "callback" in lower: return "callback"
    if "pl011" in lower: return "pl011"
    if "framebuffer" in lower and "enabled" in lower: return "framebuffer"
    if "semihosting" in lower and "mirrored" in lower: return "semihosting"
    if "memory diagnostic ring only" in lower or "memory ring" in lower: return "memory-ring"
    return "none"


def _detect_interrupt(text: str) -> str:
    lower = text.lower()
    if "gicv3" in lower: return "gicv3"
    if "gicv2" in lower: return "gicv2"
    if "loader callbacks" in lower: return "callback"
    if "polling-only" in lower: return "polling"
    return "none"
...
def check_profile(profile: dict[str, Any], diagnostics: pathlib.Path) -> dict[str, Any]:
    ...
    names = [
        "platform-info", "exception-level", "system-registers", "handoff-info",
        "services", "console-services", "memory-check", "device-tree-summary",
        "timer-test", "irq-controller", "watchdog-services", "framebuffer-info",
        "probe-status", "compatibility", "boot-modules",
    ]
    texts = {name: _read(directory, name) for name in names}
    entries: list[MatrixEntry] = []

    handoff = texts["handoff-info"]
    entries.append(_entry("Boot handoff", "PASS" if "Version:" in handoff else "FAIL",
                          "validated handoff metadata present" if "Version:" in handoff else "no active handoff"))

    el_match = re.search(r"EL([0-3])", texts["exception-level"] or texts["platform-info"])
    if el_match:
        observed_el = int(el_match.group(1))
        entries.append(_entry("Exception level", "PASS" if observed_el == profile["expected_el"] else "FAIL",
                              f"expected EL{profile['expected_el']}, observed EL{observed_el}"))
    else:
        entries.append(_entry("Exception level", "NOT TESTED", "no parseable exception-level record"))
    ...
    if expected_console == "none":
        ...
        console_state = "PASS"
    entries.append(_entry("Console output", console_state,
    ...
    entries.append(_entry("Console input",
                          "PASS" if console_input else ("FAIL" if input_required else "UNSUPPORTED"),
                          "loader/platform input service" if console_input else "no input service reported"))
    ...
    memory_ok = all(token in texts["memory-check"].lower() for token in (
        "memory map: ok", "handoff:    ok", "heap:       ok", "stack:      ok"
    ))
    entries.append(_entry("Memory ownership", "PASS" if memory_ok else "FAIL",
    ...
    entries.append(_entry("Device tree", "PASS" if device_tree_active else
                          ("FAIL" if device_tree_required else "PARTIAL"),
    ...
    if expected_irq == "none":
        ...
        irq_state = "NOT TESTED" if "delivery: masked" in texts["irq-controller"].lower() else "PASS"
    else:
        irq_state = "FAIL"
    entries.append(_entry("Interrupt controller", irq_state,
                          f"expected {expected_irq}, observed {detected_irq}"))

    watchdog_text = texts["watchdog-services"].lower()
    watchdog_available = "configure: available" in watchdog_text and "kick: available" in watchdog_text
    entries.append(_entry("Watchdog", "NOT TESTED" if watchdog_available else "UNSUPPORTED",
                          "services present but unchanged" if watchdog_available else "callbacks unavailable"))

    framebuffer_text = texts["framebuffer-info"]
    framebuffer_available = bool(re.search(r"^Framebuffer:\s+0x", framebuffer_text, re.MULTILINE))
    ...
    if framebuffer_available:
        ...
        if framebuffer_spec is not None:
            ...
            if allowed_formats:
                framebuffer_matches &= pixel is not None and pixel.group(1) in allowed_formats
    ...
    modules_locked = "Modules:         LOCKED" in texts["probe-status"] or "Modules" in texts["compatibility"] and "LOCKED" in texts["compatibility"]
    entries.append(_entry("Modules", "LOCKED" if modules_locked else "PARTIAL",
                          "defensive mode" if modules_locked else "lock state not confirmed"))

    for service in profile["required_services"]:
        entries.append(_entry(f"Service: {service}", "PASS" if _service_supported(service, texts) else "FAIL",
                              "required by board profile"))

    states = [entry.state for entry in entries]
    result = "fail" if "FAIL" in states else ("partial" if any(state in {"PARTIAL", "NOT TESTED", "LOCKED", "UNSUPPORTED"} for state in states) else "pass")
    return {
        "schema_version": 1,
...
def import_probe_results(profile: dict[str, Any], diagnostics: pathlib.Path) -> dict[str, Any]:
...
def format_matrix(report: dict[str, Any]) -> str:
    lines = [f"Profile: {report.get('profile', 'unknown')}", f"Result:  {report.get('result', 'unknown').upper()}", ""]
    for item in report.get("matrix", []):
