...
def prepare(args: argparse.Namespace) -> int:
    ...
    result = {
        ...
        "image": {
            key: image.get(key)
            for key in ("family", "sha256", "size", "component_count")
        },
...
def forward(module: str, arguments: list[str]) -> int:
    return subprocess.run(
        [sys.executable, str(ROOT / "host" / module), *arguments], cwd=ROOT, check=False
    ).returncode
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "doctor":
            return forward("device_tools.py", ["doctor", *args.arguments])
