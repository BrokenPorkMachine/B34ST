...
REGION_TYPE_IDS = {"usable": 1, "reserved": 2, "mmio": 3,
                   "monitor": 4, "framebuffer": 5}
...
MODULE_CAP_BITS = {"console": 1, "log": 2, "time": 4, "dt": 8,
                   "state": 16, "host": 32}
HANDOFF_FLAGS = {
    "device-tree": 1 << 1,
    "framebuffer": 1 << 2,
...
class ElfImage:
    def __init__(self, path: pathlib.Path):
        ...
        if phentsize != PROGRAM_HEADER.size or phnum == 0:
            raise LoaderSimulationError("generic image has an unsupported program-header table")
        ...
        for index in range(phnum):
            ...
            if p_type != PT_LOAD:
                continue
            if memsz < filesz or offset > len(self.data) or filesz > len(self.data) - offset:
                raise LoaderSimulationError(f"ELF load segment {index} is malformed")
            payload = self.data[offset:offset + filesz]
            segments.append(ElfSegment(index, offset, vaddr, paddr, filesz,
                                       memsz, flags, alignment, _sha256(payload)))
        if not segments:
            raise LoaderSimulationError("generic image contains no loadable segments")
        self.segments = tuple(segments)
        executable = [s for s in segments if s.flags & PF_X and
                      s.load_address <= self.entry < s.load_address + s.memory_size]
        if not executable:
            raise LoaderSimulationError("ELF entry point is outside executable load segments")


class SparseAllocator:
    def __init__(self, regions: list[dict[str, Any]], occupied: Iterable[tuple[int, int]]):
        self.regions = regions
        self.occupied = sorted(list(occupied))

    def allocate(self, size: int, *, alignment: int = 16,
                 allowed_types: tuple[str, ...] = ("usable", "reserved")) -> int:
        if size <= 0:
        ...
        for region in self.regions:
            ...
            while start + size <= end:
                collision = next(((a, b) for a, b in self.occupied
                                  if start < b and a < start + size), None)
...
def _region_cover(regions: list[dict[str, Any]], base: int, size: int,
                  required: set[str], types: set[str] | None = None) -> bool:
    end = base + size
...
def _callback_names(document: dict[str, Any]) -> list[str]:
    ...
    mapping = {
        ...
        "interrupt-controller": ("interrupt_ack", "interrupt_complete", "interrupt_set_enabled"),
...
def _pack_services(document: dict[str, Any], callbacks: dict[str, int]) -> bytes:
    ...
    values = [
        1, SERVICE_TABLE_SIZE, capability_mask, 0,
        callbacks.get("console_write", 0), callbacks.get("console_read", 0),
        callbacks.get("console_flush", 0), 0,
        callbacks.get("framebuffer_flush", 0), 0,
        callbacks.get("interrupt_ack", 0), callbacks.get("interrupt_complete", 0),
        callbacks.get("interrupt_set_enabled", 0), 0,
        callbacks.get("watchdog_configure", 0), callbacks.get("watchdog_kick", 0), 0,
    ]
...
def _pack_handoff(document: dict[str, Any], pointers: dict[str, int],
                  callback: dict[str, int], module_count: int) -> bytes:
    ...
    data += struct.pack(
        "<QIIQQQIIQQQQ",
        HANDOFF_MAGIC, 4, HANDOFF_SIZE,
        _number(monitor["base"]), _number(monitor["size"]),
        pointers["regions"], len(document["regions"]), 0,
        pointers.get("dtb", 0), pointers.get("dtb_size", 0), 0, 0,
    )
    data += struct.pack(
        "<QQIIQIIIIQ",
        flags, pointers.get("modules", 0), module_count, 0,
        _number(framebuffer.get("base", 0)), framebuffer.get("width", 0),
        framebuffer.get("height", 0), framebuffer.get("pixels_per_row", 0),
        framebuffer.get("pixel_format_id", 0), 0x46425233344B4552,
    )
    data += struct.pack(
        "<" + "Q" * 9,
        callback.get("runtime_getc", 0), 0,
        callback.get("timer_read", 0), callback.get("timer_frequency", 0), 0,
        callback.get("reboot", 0), callback.get("halt", 0), 0,
        0x41524D363447454E,
    )
    data += struct.pack(
        "<QIIIIQIIIHHIQQQ",
        pointers.get("services", 0), SERVICE_TABLE_SIZE if pointers.get("services") else 0,
        document["entry_exception_level"], document["page_granule"],
        document["boot_cpu_id"], _number(framebuffer.get("size", 0)),
        framebuffer.get("bytes_per_pixel", 0), framebuffer.get("rotation", 0),
        policy.get("allowed_capability_mask", 0), policy.get("dynamic_slots", 0), 0,
        policy.get("instruction_budget", 0), _number(document["firmware_revision"]), 0, 0,
    )
...
def simulate_loader(handoff_path: pathlib.Path, image_path: pathlib.Path,
                    output: pathlib.Path, *, dtb_path: pathlib.Path | None = None,
                    module_paths: Iterable[pathlib.Path] = ()) -> dict[str, Any]:
    ...
    checks: list[Check] = [
        ...
        Check("elf-architecture", "supported", "ELF64 little-endian AArch64 image validated",
    ...
    for segment in image.segments:
        ...
        placements.append(Placement(
            f"elf-segment-{segment.index}", "monitor-segment", base,
            segment.memory_size, segment.sha256, image_path.name,
        ))
    checks.append(Check("monitor-placement", "supported",
    ...
    if callback_names:
        ...
        for name in callback_names:
            ...
            callback_blob += struct.pack("<II", 0xD421E240, 0xD65F03C0)  # brk #0xf12; ret
        if callback_base + len(callback_blob) > monitor_end:
            raise LoaderSimulationError("monitor region has no room for synthetic callback stubs")
        occupied.append((callback_base, callback_base + len(callback_blob)))
        placements.append(Placement("callback-stubs", "synthetic-executable-stubs",
                                    callback_base, len(callback_blob),
                                    _sha256(callback_blob), None))
        checks.append(Check("callback-materialization", "not-tested",
        ...
        checks.append(Check("callback-materialization", "unsupported",
                            "handoff design declares no callback services"))

    allocator = SparseAllocator(regions, occupied)
    region_blob = b"".join(struct.pack(
        "<QQII", _number(item["base"]), _number(item["size"]),
        REGION_TYPE_IDS[item["type"]], item["attribute_mask"]
    ) for item in regions)
    region_address = allocator.allocate(len(region_blob), alignment=8)
    placements.append(Placement("memory-regions", "handoff-metadata", region_address,
                                len(region_blob), _sha256(region_blob), None))
    ...
    if dtb_path is not None:
        ...
        placements.append(Placement("device-tree", "fdt", dtb_address, len(dtb),
                                    _sha256(dtb), dtb_path.name))
        checks.append(Check("device-tree", "supported", "DTB header, size, and placement validated",
                            {"base": f"0x{dtb_address:x}", "size": len(dtb)}))
    else:
        checks.append(Check("device-tree", "not-tested", "no DTB was supplied to the simulator"))
    ...
    for raw_path in module_paths:
        ...
        if not payload or len(payload) > MAX_BOOT_MODULE_SIZE:
            raise LoaderSimulationError(f"boot module {path} must contain 1..{MAX_BOOT_MODULE_SIZE} bytes")
        address = allocator.allocate(len(payload), alignment=16)
        module_records.append((path, payload, address))
        placements.append(Placement(path.name, "boot-module", address, len(payload),
                                    _sha256(payload), path.name))
    if module_records:
        checks.append(Check("boot-modules", "supported", "boot modules fit granted readable memory",
                            {"count": len(module_records)}))
    else:
        checks.append(Check("boot-modules", "not-tested", "no boot modules were supplied"))

    module_blob = b"".join(struct.pack("<QQII", address, len(payload), 1, 0)
                           for _, payload, address in module_records)
    module_address = allocator.allocate(len(module_blob), alignment=8) if module_blob else 0
    if module_blob:
        placements.append(Placement("boot-module-table", "handoff-metadata", module_address,
                                    len(module_blob), _sha256(module_blob), None))

    service_blob = _pack_services(document, callbacks) if document.get("services") else b""
    service_address = allocator.allocate(len(service_blob), alignment=8) if service_blob else 0
    if service_blob:
        placements.append(Placement("platform-services", "handoff-metadata", service_address,
                                    len(service_blob), _sha256(service_blob), None))
    ...
    placements.append(Placement("handoff-v4", "handoff-structure", handoff_address,
                                len(handoff_blob), _sha256(handoff_blob), None))
    ...
    if callback_blob:
        artifacts.append(_write_blob(output, "callback-stubs.bin", bytes(callback_blob)))
    if service_blob:
    ...
    for path, payload, address in module_records:
        artifacts.append(_write_blob(output, f"module-{address:016x}-{path.name}", payload))

    for region in regions:
        attrs = set(region["attributes"])
        if region["type"] in {"usable", "mmio"} and "execute" in attrs:
            checks.append(Check("writable-executable-regions", "unsafe",
                                f"{region['type']} region is executable",
                                {"base": region["base"], "size": region["size"]}))
            break
    else:
        checks.append(Check("writable-executable-regions", "supported",
                            "no usable or MMIO region is executable"))

    runtime = set(document.get("runtime_callbacks", []))
    for feature in ("input", "timer", "power"):
        checks.append(Check(f"runtime-{feature}",
                            "not-tested" if feature in runtime else "unsupported",
                            ("callback structure materialized but not executed" if feature in runtime
                             else "callback not declared by the loader design")))
    if "interrupt-controller" in document.get("services", []):
        checks.append(Check("interrupt-controller", "not-tested",
                            "IRQ callbacks materialized; delivery requires a runtime target"))
    else:
        checks.append(Check("interrupt-controller", "unsupported",
                            "loader design has no interrupt-controller service"))

    normalized_path = output / "normalized-handoff.json"
    normalized_path.write_text(json.dumps(document, indent=2, sort_keys=True) + "\n",
                               encoding="utf-8")
    artifacts.append({"path": normalized_path.name,
    ...
    sparse_plan = {
        ...
        "regions": document["regions"],
        "placements": [asdict(item) | {"base": f"0x{item.base:x}"}
                       for item in sorted(placements, key=lambda value: value.base)],
    }
    sparse_path = output / "sparse-memory.json"
    sparse_path.write_text(json.dumps(sparse_plan, indent=2, sort_keys=True) + "\n",
                           encoding="utf-8")
    artifacts.append({"path": sparse_path.name,
                      "size": sparse_path.stat().st_size,
                      "sha256": _sha256(sparse_path.read_bytes())})

    invalid_or_unsafe = [item for item in checks if item.status in {"invalid", "unsafe"}]
    report = {
        "schema_version": 1,
        "project": "FBR34KER",
        "version": "0.3.0",
        ...
        "image": {"path": str(image_path), "sha256": _sha256(image.data),
