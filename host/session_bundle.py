...
STANDARD_FILES = (
    "session.json", "summary.json", "device.json", "profile.json",
    "image-manifest.json", "transfer.log", "console.log",
    "boot-evidence.json", "trace.json", "crash-report.json",
)
...
def write_session_bundle(path: pathlib.Path, values: dict[str, Any]) -> None:
    files = {name: _encode(values.get(name, {} if name.endswith(".json") else ""))
...
def verify_session_bundle(path: pathlib.Path) -> dict[str, object]:
    ...
    for line in files.get("checksums.sha256", b"").decode("ascii", "strict").splitlines():
    ...
    for name in STANDARD_FILES:
        if name in files and expected.get(name) != hashlib.sha256(files[name]).hexdigest():
            failures.append(name)
    return {"valid": not missing and not failures, "missing": missing,
            "checksum_failures": failures, "members": sorted(files)}
