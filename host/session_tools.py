    ...
    if not console_only:
        ...
        for stage in session.get("stages", []):
            if isinstance(stage, dict):
                print(
                    f"[{stage.get('status', 'unknown')}] {stage.get('stage', 'unknown')}"
                )
        print("--- console ---")
...
def crash_decode(path: pathlib.Path) -> int:
    ...
    value = (
        json_member(files, "crash-report.json")
...
def trace_timeline(path: pathlib.Path) -> int:
    ...
    for index, record in enumerate(records):
        if isinstance(record, dict):
            print(
                f"{index:04d} {record.get('operation', record.get('event', 'unknown'))}: {record.get('status', '')} {record.get('detail', '')}".rstrip()
            )
    return 0
...
def parser() -> argparse.ArgumentParser:
    ...
    replay.add_argument("--console-only", action="store_true")
    crash = sub.add_parser("crash-decode")
    crash.add_argument("input", type=pathlib.Path)
    trace = sub.add_parser("trace-timeline")
    trace.add_argument("input", type=pathlib.Path)
    return root
...
def main(argv=None) -> int:
    ...
    try:
        if args.command == "inspect":
            return inspect_session(args.bundle)
        if args.command == "replay":
            return replay_session(args.bundle, console_only=args.console_only)
        if args.command == "crash-decode":
            return crash_decode(args.input)
