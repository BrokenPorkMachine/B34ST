...
def main():
    ...
    p.add_argument(
        "--profile",
        type=pathlib.Path,
        default=pathlib.Path("profiles/apple-a13-recovery.json"),
    )
    p.add_argument(
        "--device-info",
        type=pathlib.Path,
    ...
    success = run(
        [
            *common,
            "--state-dir",
            str(out / "success-state"),
            "--evidence",
            str(out / "success-evidence.zip"),
        ],
        0,
    )
    failed = run(
        [
            *common,
            "--state-dir",
            str(out / "recovery-state"),
            "--inject-failure",
            "timer",
            "--evidence",
            str(out / "failure-evidence.zip"),
        ],
        1,
    )
    recovered = run(
        [
            *common,
            ...
            str(out / "recovery-state"),
            "--evidence",
            str(out / "recovered-evidence.zip"),
        ],
        0,
    )
    ...
    summary = {
        ...
        "failure_observed": not failed["passed"],
        "reset_invalidated_authorization": failed[
            "authorization_invalidated_after_recovery"
        ],
        "recovered_after_reauthorization": recovered["passed"],
    }
    (out / "simulation-summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n"
    )
    print(out / "simulation-summary.json")
    return 0
