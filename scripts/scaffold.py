...
def safe_name(value: str) -> str:
    if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_-]{0,47}", value):
        raise ValueError(
            "name must begin with a letter and contain only letters, digits, '_' or '-'"
        )
    return value


def render_tree(
    kind: str, name: str, destination: pathlib.Path, force: bool = False
) -> list[pathlib.Path]:
    source = TEMPLATES / kind
    if not source.is_dir():
        raise ValueError(f"missing {kind} template")
    ...
    for item in sorted(source.rglob("*")):
        if item.is_dir():
            continue
        rel = pathlib.Path(
            item.relative_to(source).as_posix().replace("__name__", identifier)
        )
