...
from b34st.control_panel import _header, _enhanced_prompt, _help_for_category, _smart_default_recommendation, Session, _main_menu_options
...
def test_comprehensive_menu_capabilities():
    ...
    print(f"  Categories Covered: {len(covered_categories)}/{len(essential_categories)}")
...
def test_smart_defaults_coverage():
    ...
    return passed_categories >= tested_categories * 0.8  # Allow some categories to have no defaults
...
def test_automation_exposure():
    ...
    automation_keywords = [
        "automation", "scriptable", "guided", "planner", "orchestration",
        "suggest", "assist", "default", "recommended", "quick"
    ]
    ...
    print(f"\nAutomation Coverage:")
    print(f"  Categories with Automation: {categories_with_automation}/{total_categories}")
...
def test_education_features():
    ...
    education_keywords = [
        "guide", "plan", "validate", "explain", "illustrate", "demonstrate",
        "step-by-step", "tutorial", "walkthrough", "how to", "what is"
    ]
    ...
    for key, label, desc in menu_options:
        ...
        try:
            ...
            print(f"  ✓ Help header provides education")
            
            # Test without help
            _header(session, label, show_help=False)
            print(f"  ✓ Clean header also available")
    ...
    header_success_rate = successful_headers / tested_headers * 100 if tested_headers > 0 else 0
    
    print(f"\nEducation Features Coverage:")
    print(f"  Categories with Educational Content: {educational_categories}/{len(menu_options)}")
