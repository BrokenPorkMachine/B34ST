...
def test_enhanced_prompt():
    ...
    # Test that _enhanced_prompt function exists and works
    assert callable(_enhanced_prompt), "_enhanced_prompt should be callable"
    print("  ✓ _enhanced_prompt function exists")
...
def test_smart_defaults():
    ...
    recommendation2 = _smart_default_recommendation("A12+ USBliter8 — Pwn, Inspect, Jailbreak", None)
...
def test_menu_categories():
    ...
    main_menu_options = [
        ("1", "External hardware / USBliter8 / first-stage execution", "Pwn, inspect, and jailbreak A12+ devices via USBliter8"),
        ("2", "Load images, next stages, modules, or deployment", "Boot chain, deployment, modules, runtime"),
        ("3", "Authorized runtime modifications and evidence", "Kernel patches, secure boot bypass, persistence"),
        ("4", "Build, test, and QEMU simulation", "Host check, build, test, simulate safely"),
        ("5", "Runtime console / logger / shell", "Interactive shell, logging, exploration"),
        ("6", "Evidence, validation, and release", "Validate sessions, generate reports, release gate"),
        ("7", "Targeted IPSW downloads, upgrades, and tethered downgrades", "Firmware catalog, signed updates, guide"),
        ...
        ("11", "Forensics and data acquisition", "iCloud/Keychain, activation, passcode, memory/storage"),
        ("12", "CVE database & exploit chain planner", "Search exploits, plan chains, suggest attacks"),
    ...
    print(f"  ✓ Automation readiness: {sum(auto_indicators)}/15 categories have automation ready options")
