...
import json, pathlib, shlex, sys, tempfile, unittest
...
class BridgeProtocolTests(unittest.TestCase):
    ...
    def test_persistent_chunked_bridge_session(self):
        with tempfile.TemporaryDirectory() as d:
            root=pathlib.Path(d); _,image,command=self.fixture(root)
            adapter=BridgeFirstStageAdapter(command,timeout=10,chunk_size=1024)
            try:
                identity=adapter.identify(); self.assertIn('chunked-upload',identity.capabilities)
                adapter.authorize('authorized-bridge-session')
                uploaded=adapter.upload(image); self.assertEqual(uploaded['sha256'],adapter.verify_image(image)['image']['sha256'])
                adapter.start(); adapter.probe_stage('console')
                console=adapter.read_console(0); self.assertTrue(console['lines'])
                reset=adapter.reset('authorized-bridge-session'); self.assertFalse(reset['authorized'])
                with self.assertRaises(AdapterError): adapter.start()
            finally:
                adapter.close()
    def test_bridge_rejects_bad_chunk_size(self):
        with tempfile.TemporaryDirectory() as d:
            root=pathlib.Path(d); _,_,command=self.fixture(root)
            with self.assertRaises(AdapterError): BridgeFirstStageAdapter(command,chunk_size=999999)
