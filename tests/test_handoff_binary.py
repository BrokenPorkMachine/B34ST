...
from handoff_binary import build_blob, inspect_blob, roundtrip_blob, HandoffBinaryError  # noqa: E402
...
class HandoffBinaryTests(unittest.TestCase):
    ...
    def test_tamper_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            p = pathlib.Path(td) / "h.fbhb"
            data = bytearray(build_blob(ROOT / "examples/handoff-v4.json"))
            data[-1] ^= 1
            p.write_bytes(data)
            with self.assertRaises(HandoffBinaryError):
                inspect_blob(p)
