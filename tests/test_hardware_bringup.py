...
class HardwareBringupTests(unittest.TestCase):
    ...
    def test_complete_simulated_bringup(self):
        with tempfile.TemporaryDirectory() as d:
            root = pathlib.Path(d)
            image, device = self.fixture(root)
            evidence = root / "evidence.zip"
            result = subprocess.run(
                self.command(root, image, device, "--evidence", str(evidence)),
                cwd=ROOT,
                text=True,
                capture_output=True,
            )
    ...
    def test_failure_recovers_and_requires_reauthorization(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(summary["authorization_invalidated_after_recovery"])
            result2 = subprocess.run(
                self.command(root, image, device),
                cwd=ROOT,
                text=True,
                capture_output=True,
            )
    ...
    def test_persistent_bridge_exact_profile(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with zipfile.ZipFile(evidence) as bundle:
                ...
                self.assertIn("console.log", bundle.namelist())
