...
from hardware_profile import load_profile, import_probe_results, validate_profile
class ProfileImportTests(unittest.TestCase):
    def test_import_never_promotes_untested_services(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for name,text in records.items(): (d/f"{name}.txt").write_text(text)
            reviewed=import_probe_results(load_profile(ROOT/"examples/hardware-profile.json"),d)
